...
while True:
    with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as s:
        ...
        while len(size_data) < 4:
            ...
            thedata = '';
            for i in range(0, 12):
                thedata += 'pwm_monitor,source=pwm_monitor,name=roi' + str(i) + ' value=' + str(arr[i]) + '\n'
            print(thedata)
        ...
        while len(size_data) < 4:
            ...
            thedata = '';
            for i in range(0, 12):
                thedata += 'pwm_monitor,source=pwm_monitor,name=pwm' + str(i) + ' value=' + str(arr[i]) + '\n'
            print(thedata)
            session.post(url, data=thedata.encode())
        ...
        while len(size_data) < 4:
            ...
            thedata = '';
            for i in range(0, 12):
                thedata += 'pwm_monitor,source=pwm_monitor,name=pidval' + str(i) + ' value=' + str(arr[i]) + '\n'
            print(thedata)
            session.post(url, data=thedata.encode())
