...
cat ${database} | grep -v "#" | while read line; do
    ...
    timeout 10m root -b -q -l "${ALCOR_DIR}/measure/2024-laser-window/drawing-routines/signal.C(\"${dirname}/decoded\", \"${channel}\", ${reference})" &> /dev/null &
    sleep 0.1
done
