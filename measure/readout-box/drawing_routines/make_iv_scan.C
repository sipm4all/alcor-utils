...
{
  TVirtualFitter::SetMaxIterations(1.e6);
  ...
  auto f = new TF1("f", "expo(0) + pol0(2) + (x > [3]) * ( [4] * pow(abs(x - [3]), [5]) + [6] * pow(abs(x - [3]), [7]) )", 0., 100.);
  ...
  int res = g->Fit(f, "0", "", 0., vstart + 3.);
  std::cout << " --->>> " << res << std::endl;

  f->SetRange(0., vstart + 5.);

  if (oneshot || fabs(f->GetParameter(3) - vstart) < 0.01) break;
