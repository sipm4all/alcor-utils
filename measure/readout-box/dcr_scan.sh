...
for BIAS_VOLTAGE in $BIAS_VOLTAGES; do
    ...
    ### set Vbias
    BIAS_DAC=$(/au/masterlogic/hvcalib/hvcalib-malaguti.sh ${CARRIER} $BIAS_VOLTAGE | grep dac | awk '{print $2}')
    if [ -z "$AU_DRYRUN" ]; then
...
recipients="roberto.preghenella@bo.infn.it matilde.zucchini@studio.unibo.it"
