...
outputdir=/home/eic/DATA/2024-laser-window/actual/latest/subruns/$runname
mkdir -p $outputdir
...
ln -sfn $outputdir /home/eic/DATA/2024-laser-window/actual/latest/subruns/latest
...
### start of run/spill
sleep 1
/au/readout/python/interprocess.py 0x011
sleep 1
for ispill in $(seq 1 $nspill); do
    echo " --- start of spill: ${ispill} / ${nspill}"
    /au/readout/python/interprocess.py 0x013
    sleep 0.1
    echo " --- end of spill "
    /au/readout/python/interprocess.py 0x011
    sleep 0.1
