...
namespace sipm4eic
{
  ...
  public:
    ...
    static bool close_to_cut(int _index, int _fine) {
      if (_fine == 0.) return false;      
      double min = fine_min[_index];
      double max = fine_max[_index];
      if (min == 0. || max == 0.) return false;
      double cut = 0.5 * (max + min) - 0.5;
      if (std::fabs(_fine - cut) < 1.) return true;
      return false;
    }
    
    static double fine_phase(int _index, int _fine) {
      if (_fine == 0.) return 0.;
      double min = fine_min[_index];
      double max = fine_max[_index];
      if (min == 0. || max == 0.) return 0.;
      double phase = (_fine - min) / (max - min);
      if (_fine >= 0.5 * (max + min)) phase -= 1.;
      return phase;
    }
    ...
    bool is_deadbeef()    const { return ( (rollover << 15) + coarse ) == 0xdeadbeef; };
  ...
  const int data::do2eo[32] = {27, 23, 26, 22, 25, 21, 24, 20, 28, 16, 29, 17, 30, 18, 31, 19, 3, 15, 2, 14, 1, 13, 0, 12, 4, 8, 5, 9, 6, 10, 7, 11};
