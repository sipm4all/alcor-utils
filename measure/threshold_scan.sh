...
### thresholds
[ -z "$AU_SCAN_THRESHOLDS" ] && THRESHOLDS=$(seq 0 3 63) || THRESHOLDS=${AU_SCAN_THRESHOLDS}
...
for PULSE_VOLTAGE in $PULSE_VOLTAGES; do
	...
	for THRESHOLD in $THRESHOLDS; do
		...
		[ -z "$OUTPUT" ] && continue
		echo "bias_voltage = $BIAS_VOLTAGE bias_dac = $BIAS_DAC pulse_voltage = $PULSE_VOLTAGE $OUTPUT" | tee -a $FILENAME
	    done
...
/au/measure/tree.sh $FILENAME bias_voltage/F bias_dac/I pulse_voltage/I \
