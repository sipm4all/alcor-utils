...
### length of the SOFT reset (clock cycles)
soft_length = 8 ### 8 is now the new GOOD, 10 is the OLD lenght, 4 is NOT OK.
...
def globalCommands(hw,chip,delay,phase):
...
#    stat=sendCtrl(hw,chip,cmdSetSyncResetPhase,phase)
    stat=sendCtrl(hw,chip,cmdSetSyncResetPhase,phase | ( (soft_length - 1) << 4 ) ) ### R+SOFT
    stat=sendCtrl(hw,chip,cmdDoReset,0)
    stat=sendCtrl(hw,chip,cmdResetCounters,0)
    stat=sendCtrl(hw,chip,cmdSetTxEventFilters,0xF)
    return 0

def init2(hw,chip,delay,lmask,phase):
    status=0
