...
def readECCR(hw,chip,mode):
    for ad in range(ECCRregs):
        d=mode
        d=programPtrReg(hw,chip,ECCR,ad,d,RDREG)
        print("ECCR Read ",ad,": ",hex(d))
    return 0
...
def init2(hw,chip,delay,lmask):
    ...
    for ad in range(lanes):
        if ( (1<<ad) & lmask):
           ECCR_Init=(ECCR_default|ECCR_SER_align)&(~ECCR_RAWMODE)        
           print("Aligning at 8 bit lane ",ad)
           print("  switching ECCR to align (8 bit)") 
           ...
           adA=ad*2
           adB=adA+1
           err1=sendCtrl(hw,chip,cmdRead8B10BErrCounters,adA)
           err2=sendCtrl(hw,chip,cmdRead8B10BErrCounters,adB)
#           print("Error counters after alignment ",err1,err2)
           print("Aligning at 32 bit lane ",ad)
           print("  issueing TxWordAlign") 
           stat=sendCtrl(hw,chip,cmdStartTxWordAlign,ad)
           print("  switching ECCR to align (32 bit)") 
           d=programPtrReg(hw,chip,ECCR,ad,ECCR_Init,SETREG)
           stat=sendCtrl(hw,chip,cmdReadTxWordAlignResult,ad)
           print("  StartTxWordAlignResult status ",hex(stat))
           if ( (stat & 0x1) ):
...
def init(hw,chip,delay):
    ...
    if (adir == None):
        ...
        print("[AlcorLib] Loading conf from ",adir)
    ctrl(hw,chip,HardRESET)
#    setup(hw,chip,1,0)   # to give BCR/PCR 

    ECCR_Init=0
    readECCR(hw,chip,ECCR_Init)
    ECCR_Init=(ECCR_default|ECCR_SER_align)&(~ECCR_RAWMODE)
#    ECCR_Init=0xb000|ECCR_SER_align
    setupECCR(hw,chip,ECCR_Init)
    readECCR(hw,chip,ECCR_Init)
    status=loadSequence(hw,chip,adir+"/alcor-sync.cfg."+delay)
    if (status != 0):
        print(" Failed global SYNC ")
        exit(1)
    readECCR(hw,chip,ECCR_Init)
    ECCR_Init=ECCR_default|ECCR_RAWMODE
    setupECCR(hw,chip,ECCR_Init)
    ...
    readECCR(hw,chip,ECCR_Init)
#    exit(0) 
    status=loadSequence(hw,chip,adir+"/alcor-align-mod.cfg")
    readECCR(hw,chip,ECCR_Init)
    if (status == 1):
    ...
    if (status == 2):
        ...
        return 1
    ECCR_Init=(ECCR_default|ECCR_SER_align)
    for ad in range(lanes):
        print("Aligning at 32 bit lane ",ad)
        val=encodeCtrl(cmdStartTxWordAlign,ad)
        stat=ctrl(hw,chip,val)
        print("  StartTxWordAlign status ",hex(stat))
        print("  switching ECCR to align") 
        d=programPtrReg(hw,chip,ECCR,ad,ECCR_Init,SETREG)
        val=encodeCtrl(cmdReadTxWordAlignResult,ad)
        stat=ctrl(hw,chip,val)
        print("  StartTxWordAlignResult status ",hex(stat))
...
#                if ( (ctrlWord & 0xF) == 0):
                  if ( (ipstat & 0xF) != 0xF ):
                      print("FATAL (SyncTx): ALCOR lanes not correctly synced ",hex(ipstat))
                      return 1
            ### R+fix to detect lane not correctly aligned
            if ctrlCmd == cmdReadTxWordAlignResult:
                 if ( ipstat & 0x1 ): 
                    print("FATAL (ReadTxWord): ALCOR lane ", (ctrlWord & 0x3), " not correctly synced. Status: ",(ipstat & 0x7))
                    status = 2
#                if not has_stored_lane_check:
#                    stored_lane_check = ipstat
#                    has_stored_lane_check = True
#                elif ipstat != stored_lane_check:
#                    print("FATAL (ReadTxWord): ALCOR lanes not correctly synced ",hex(ipstat)," stored: ",hex(stored_lane_check))
#                    return 2
    if (status == 0): 
