...
# echo $chip $lane $eccr $bcr $pcr
   ...
   if [ $ldec -ne 0 ]; then
	   ...
	   ${ALCOR_DIR}/control/alcorInit.py $CONN kc705 -c $chip -l $lane --phase $phi -d $delay $SWITCH --eccr 0xb81b --bcrfile ${BCR_DIR}/$bcr.bcr --pcrfile ${PCR_DIR}/$pcr.pcr & 
       else
	   ${ALCOR_DIR}/control/alcorInit.py $CONN kc705 -c $chip -l $lane --phase $phi  -d $delay $SWITCH --eccr $eccr --bcrfile ${BCR_DIR}/$bcr.bcr --pcrfile ${PCR_DIR}/$pcr.pcr &
       fi
       if [ $runNr -ne 0 ]; then
